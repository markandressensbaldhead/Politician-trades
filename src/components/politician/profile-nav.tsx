...
export function ProfileNav({ hasTrades = true, className }: ProfileNavProps) {
  ...
  return (
    <nav
      className={cn(
        "sticky top-16 z-30 -mx-4 border-b border-border/60 bg-background/90 px-4 backdrop-blur-md sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8",
        className
      )}
      ...
      <div className="flex gap-1 overflow-x-auto py-2.5 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {links.map((link) => (
          <a
            key={link.id}
            href={`#${link.id}`}
            className="shrink-0 rounded-md px-3.5 py-1.5 text-sm font-medium text-muted-foreground transition-colors hover:bg-secondary/80 hover:text-foreground"
          >
            {link.label}
          </a>
        ))}
